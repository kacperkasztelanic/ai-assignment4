...
class Ransac:
    def __init__(self, paths, filtered_pairs, all_pairs, heuristic=DistanceHeuristic.default_none, lower_limit=0.1,
                 upper_limit=0.3):
    ...
    def ransac_model(self, size, no_draws, max_error):
        ...
        for i in range(no_draws):
            ...
            while model is None:
                ...
                elif self.heuristic == DistanceHeuristic.euclidean_distance_heuristic:
    ...
    def calc_model(samples):
        ...
        if len(samples) == 3:
            a = Ransac.affine_array(x1, y1, x2, y2, x3, y3, u1, v1, u2, v2, u3, v3)
        else:
            x4 = samples[3][0].coords[0]
            y4 = samples[3][0].coords[1]
            u4 = samples[3][1].coords[0]
            v4 = samples[3][1].coords[1]
            a = Ransac.perspective_array(x1, y1, x2, y2, x3, y3, x4, y4, u1, v1, u2, v2, u3, v3, u4, v4)
        return a
    ...
    def affine_array(x1, y1, x2, y2, x3, y3, u1, v1, u2, v2, u3, v3):
        ...
        if not Ransac.is_invertible(a1):
            return None
    ...
    def calculate_ransac_pairs(self, max_error):
        ...
        for pair in self.all_pairs:
            ...
            if distance.cdist(np.reshape(pair[1].coords, newshape=(-1, 1)),
                ...
                self.ransac_pairs.append((pair[0], pair[1]))

    def get_ransac_pairs(self):
        return self.ransac_pairs
